...
JSF注册consumer,rpc http提供Ge'tQueryList接口，channel 初始化remotefile://127.0.0.1:8012/jsf/com.jd.model.predictor/new_arch

brpc周期性拉取ip list

```c++
// From global.cpp register namingservice
...
```mermaid
classDiagram
RemoteFileNamingService -->PeriodicNamingService
PeriodicNamingService --> NamingService
class NamingService {
+ RunNamingService(const char* service_name,NamingServiceActions* actions)
+ RunNamingServiceReturnsQuickly()
+ New()
}

class PeriodicNamingService {
+ GetServers(const char *service_name,std::vector<ServerNode>* servers)
+ RunNamingService(const char* service_name,NamingServiceActions* actions)                           
}

class RemoteFileNamingService {
+ GetServers(const char *service_name,std::vector<ServerNode>* servers)
+ NamingService* New() 
+ Destroy()
std::unique_ptr<Channel> _channel; // brpc call to get server list
std::string _server_addr; // http server
